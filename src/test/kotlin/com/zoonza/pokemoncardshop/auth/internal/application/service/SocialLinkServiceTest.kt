...
import com.zoonza.pokemoncardshop.auth.test.fake.FakeSocialLinkRepository
...
class SocialLinkServiceTest {
    ...
    private lateinit var socialAccountRepository: FakeSocialLinkRepository
    private lateinit var socialLinkService: SocialLinkService

    @BeforeEach
    fun setUp() {
        memberRegistrationApi = FakeMemberRegistrationApi()
        socialAccountRepository = FakeSocialLinkRepository()
        socialLinkService = SocialLinkService(
            memberRegistrationApi = memberRegistrationApi,
            socialLinkRepository = socialAccountRepository,
        )
    }

    @Test
    fun `신규 소셜 로그인 시 회원과 소셜 계정을 등록하고 회원 ID를 반환한다`() {
        val result = socialLinkService.login(
            SocialLoginCommand(
        ...
        socialAccountRepository.savedSocialLinks.shouldHaveSize(1)

        val savedSocialAccount = socialAccountRepository.savedSocialLinks.first()
    ...
    fun `기존 소셜 로그인 시 회원을 새로 만들지 않고 마지막 로그인 시간을 갱신한다`() {
        val existingSocialLink = SocialLink.register(
            memberId = 3L,
            provider = OAuth2Provider.GOOGLE,
            socialId = "google-social-id",
            createdAt = LocalDateTime.of(2026, 6, 25, 10, 30),
        )
        socialAccountRepository.addSocialAccounts(existingSocialLink)
        ...
        socialAccountRepository.savedSocialLinks.shouldHaveSize(0)
        existingSocialLink.lastLoginAt shouldNotBe null
    }
}
